...
let showData = (movies) => {
  container.innerHTML = movies
    .map(
      (m) => `
      <div class="border-2 border-amber-500 text-white p-4 rounded-lg shadow-lg w-64 flex flex-col items-center text-center">
        <img src="${m.poster_path}" alt="${m.title}" class="w-full rounded mb-2" />
        <h1 >${m.title}</h1>
...
action.addEventListener("click", () => {
  let newMovie = movies.filter((m) => m.category == "Action");
  showData(newMovie);
});


thriller.addEventListener("click", () => {
  let newMovie = movies.filter((m) => m.category == "Thriller");
  showData(newMovie);
});

animation.addEventListener("click", () => {
  let newMovie = movies.filter((m) => m.category == "Animation");
  showData(newMovie);
});

horror.addEventListener("click", () => {
  let newMovie = movies.filter((m) => m.category == "Horror");
  showData(newMovie);
});

drama.addEventListener("click", () => {
  let newMovie = movies.filter((m) => m.category == "Drama");
  showData(newMovie);
});

sci_fi.addEventListener('click', ()=>{
    let newMovie = movies.filter((m) => m.category == "Sci-Fi");
    showData(newMovie);
})